...
export const EnhancedSubjects: React.FC = () => {
  const { user } = useAuth();
  ...
  const handleSubjectClick = (subject: string) => {
    // Validate subject before navigation
    if (!subject || !availableSubjects.includes(subject)) {
      showToast({
        type: 'error',
        title: 'Subject Not Available',
        message: 'The selected subject is not available for your current class.',
        duration: 3000
      });
      return;
    }
  ...
  return (
    <div className="space-y-8 bg-student-background min-h-screen text-student-text">
      ...
      <div className="text-center">
        ...
        <div className="mt-4 inline-flex items-center px-4 md:px-6 py-2 md:py-3 rounded-full font-semibold shadow-student-card bg-student-primary text-white">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4 md:gap-6">
          {availableSubjects.map((subject, index) => {
            ...
            return (
              <div
                key={subject}
                className="student-card p-4 md:p-6 hover:shadow-student-hover transition-all duration-300 animate-student-slide-up student-hover-lift cursor-pointer"
                style={{ animationDelay: `${index * 0.1}s` }}
                onClick={() => handleSubjectClick(subject)}
              >
                <div
                  className={`w-16 h-16 md:w-20 md:h-20 mx-auto mb-4 rounded-full bg-gradient-to-r ${colorClass} flex items-center justify-center shadow-student-card hover:shadow-student-hover transition-all duration-200 hover:scale-110`}
                >
                ...
                <div className="mb-4">
                  <div className="flex justify-between text-sm mb-1">
                    ...
                    <span className="font-bold text-student-text">{progress}%</span>
                  </div>
                ...
                <div className="space-y-2 text-center">
                  <div className="text-xs md:text-sm text-student-text-secondary">
                  ...
                  <ResponsiveButton
                    ...
                    className="mt-4"
                  >
                    <span className="hidden sm:inline">Explore {displayName}</span>
      ...
      {/* Help Section */}
      <div className="student-card p-6 md:p-8 bg-gradient-to-r from-student-primary/10 to-student-accent/10 border border-student-primary/20 animate-student-slide-up" style={{ animationDelay: '0.5s' }}>
        <div className="text-center">
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
            <div className="text-center">
              <div className="w-12 h-12 md:w-16 md:h-16 mx-auto mb-3 rounded-full bg-student-primary flex items-center justify-center shadow-student-card">
                <BookOpen className="w-6 h-6 md:w-8 md:h-8 text-white" />
              </div>
              <h3 className="font-bold text-student-text mb-2 text-sm md:text-base">
                📚 Books & Chapters
              </h3>
              <p className="text-student-text-secondary text-xs md:text-sm">
                Read textbooks and study materials organized by chapters
              </p>
            </div>

            <div className="text-center">
              <div className="w-12 h-12 md:w-16 md:h-16 mx-auto mb-3 rounded-full bg-student-secondary flex items-center justify-center shadow-student-card">
            ...
            <div className="text-center">
              <div className="w-12 h-12 md:w-16 md:h-16 mx-auto mb-3 rounded-full bg-student-accent flex items-center justify-center shadow-student-card">
